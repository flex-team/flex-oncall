---
description: 아카이브 안 된 operation-notes를 일괄 조회·갱신하고, 쿡북 업데이트 후 아카이브 처리
allowed-tools: Bash, Read, Write, Edit, Grep, Glob, Task, Agent
argument-hint: (인자 없음 — 전체 활성 노트 대상)
---
...
### Step 4: 쿡북 업데이트
완료된 이슈 중 쿡북 반영 대상이 있으면 `update-cookbook.md`를 읽고 **증분 모드**를 per-ticket으로 실행한다.

1. 각 완료 노트에 대해 쿡북 반영 대상 판별 (`update-cookbook` 규칙 따름):
   - 코드 수정으로 해결 → 스킵
   - 운영 대응 / 스펙 확인 → 반영 대상
2. 반영 대상 노트가 있으면:
   - `commands/ops/update-cookbook.md`를 읽는다
   - 증분 모드 (Step 1~4)를 각 대상 티켓에 대해 수행
...
- 쿡북 관련 규칙은 `update-cookbook`을 따른다 (코드 진입점 미포함, SQL 파라미터화, 도메인 분류 등)
