---
description: 활성 노트 일괄 유지보수 + 아카이브. --rebuild 로 COOKBOOK/INDEX 전체 재구성
...
`--rebuild` 플래그를 전달하면 **COOKBOOK.md + INDEX.md 전체 재구성 모드**로 실행한다.
...
- 활성 = `{notes-dir}/` 루트에 있는 `.md` 파일 (CLAUDE.md, COOKBOOK.md, INDEX.md 제외)
- 아카이브 = `archive/` 하위 디렉토리로 이동된 노트

## Procedure

### Step 1: 활성 노트 수집
- `{notes-dir}/*.md` 목록 조회 (CLAUDE.md, COOKBOOK.md, INDEX.md 제외)
- `archive/` 하위는 제외
...
1. 각 완료 노트에 대해 쿡북 반영 대상 판별 (`update-artifacts` 규칙 따름):
   - 코드 수정으로 해결 → 쿡북 스킵 (INDEX는 갱신)
   - 운영 대응 / 스펙 확인 → COOKBOOK + INDEX 모두 반영
2. 반영 대상 노트가 있으면:
...
1. Linear 이슈 상태가 완료 (Done/Closed/Canceled)
...
4. INDEX.md 내 노트 링크도 archive/ 경로로 업데이트
5. 연관 이슈 노트의 상대 링크 업데이트
...
도메인별로 분류하여 `{notes-dir}/COOKBOOK.md` 를 전체 재작성한다.
`update-artifacts.md` 의 COOKBOOK 구조 및 규칙을 따른다.

### Step 3: INDEX.md 전체 재구성
각 노트의 제목(H1)과 처음 20줄을 읽어 요약, 도메인, 키워드를 추출한다.
도메인별로 그룹핑하여 `{notes-dir}/INDEX.md` 를 전체 재작성한다.
`update-artifacts.md` 의 INDEX 구조 및 규칙을 따른다.
...
- 아카이브 시 관련 링크(COOKBOOK.md, INDEX.md, 연관 이슈 노트) 일괄 업데이트
