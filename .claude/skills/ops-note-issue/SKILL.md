...
**도메인별 노트 템플릿 조회**: primary 도메인의 `d:tpl` 속성을 확인한다.
- `d:tpl` 값이 있으면 → `brain/note-templates/{tpl}.md` 를 읽어 추가 섹션을 파악
- `d:tpl` 값이 없으면 → `brain/note-templates/_default.md` 만 사용 (추가 섹션 없음)
- 조회한 추가 섹션은 Step 3에서 노트 생성 시 삽입한다.
...
{도메인별 추가 섹션: d:tpl이 있으면 brain/note-templates/{tpl}.md의 추가 섹션을 여기에 삽입.
 Error/Perf/Auth 타입이면 대부분 "해당 없음", Data 타입이면 상세 작성.}
