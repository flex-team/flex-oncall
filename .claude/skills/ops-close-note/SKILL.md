...
`ops-note-issue` 스킬을 읽고 **Step 1~6을 그대로 수행**한다.
...
**유지 대상**:
- 증상 1-2줄 요약
- 문제 평가 (assess-issue 산출물)
- 확정 원인
- 영향 분석 + 해결안 (impact-analyze 산출물, 있는 경우)
- 해결 조치 + PR 링크
- 참조 링크/각주
- Gherkin 시나리오
...
> 이 단계는 조사 과정에서 API 엔드포인트가 등장한 이슈에만 수행한다.
> API 엔드포인트가 전혀 없는 이슈(순수 데이터/스펙 이슈 등)는 건너뛴다.

노트에 기록된 API 엔드포인트와 domain-map.ttl의 `d:api` 를 비교하여 누락·오류를 찾는다.

1. **노트에서 API prefix 추출**: 조사 중 사용한 엔드포인트(access log 검색 URI, 사용자 제보 URL 등)의 path prefix를 수집한다.
   - 예: `/api/v2/time-tracking/work-clock/123` → prefix `/api/v2/time-tracking/work-clock`
   - 쿼리스트링·경로 변수는 제거한다

2. **domain-map.ttl `d:api` 와 비교**: 해당 도메인의 `d:api` 목록과 대조한다.

3. **결과 처리**:
   - 노트 prefix가 `d:api` 에 없으면 → **추가 제안** (자동 갱신 금지, 사용자 확인 후 진행)
   - 조사 중 해당 URI가 404였음이 노트에 기록된 경우 → `d:api` **삭제/수정 제안**
   - 일치하거나 체크할 엔드포인트가 없으면 → "변경 없음"

### Phase 4: 트리아지 분류 검증

> 사용자에게 짧게 확인하고 필요 시 triage-signals.md 를 조정하는 단계.
> 확인 없이 triage-signals.md 를 자동 수정하지 않는다.

#### 4-1. 분류 일치 확인

사용자에게 확인:
> "초기 분류 **{초기 타입}** 이 최종 결론과 일치하나요?"

- **초기 분류**: 이슈 인입 시 ops-find-domain에서 판별한 타입 (Error/Data/Perf/Auth/Spec)
- **최종 결론**: 노트의 "원인 분석"/"해결" 섹션에서 확정된 원인 성격. investigate 미실행(assess만으로 종료)인 경우, "문제 평가"의 성격 판단을 최종 결론으로 사용한다.

#### 4-2. 불일치 시 triage-signals.md 조정 제안

초기 분류와 최종 결론이 다르면, `brain/triage-signals.md` 조정을 **제안만** 한다:
- 키워드 추가/제거
- 복합 신호 패턴 추가
- 사용자 확인 후에만 실제 수정 진행
...
노트의 `d:tpl` 추가 섹션이 도메인 전환(append)되었는지 확인한다.

1. **domain-map.ttl의 `n:{ticket-id}` `d:in` 도메인**과 **노트에 삽입된 추가 섹션의 도메인 유형**을 비교
2. **전환이 있었으면**: "초기 도메인 {A} → 최종 도메인 {B} 전환. 왜 오판했는가?" 를 노트의 비고 섹션에 한 줄 기록
3. **`d:tpl` 매핑 자체가 맞지 않았으면**: `d:tpl` 값 변경을 **제안** (자동 수정 금지)

#### 4-4. 도메인별 추가 섹션 빈 항목 처리

노트의 도메인별 추가 섹션(소급 영향 범위, 배포 영향도, 법적 리스크 등)을 스캔한다.

- 내용이 채워진 항목 → 그대로 유지
- "해당 없음"으로 기입된 항목 → 그대로 유지 (확인했다는 증거)
- 아예 비어있는 항목 → "미확인"으로 표시하고, 최종 보고에서 "미확인 항목 {N}건" 안내

### Phase 5: 최종 보고

Phase 1~4 결과를 통합하여 한번에 보고한다.
...
- d:api 체크: {추가 제안 | 삭제 제안 | 변경 없음 | 해당 없음}
- {건너뜀 — 이슈 미완료}

### Phase 4: 트리아지 분류 검증
- 초기 분류: {Error | Data | Perf | Auth | Spec | 기록 없음}
- 최종 결론: {확정 원인 성격}
- 일치 여부: {일치 | 불일치}
...
### Phase 7: investigation 메트릭 기록

이슈 클로즈 시 investigation 메트릭 이벤트를 `metrics/{user}/{date}.jsonl` 에 기록한다.
- 노트에 `## 원인 분석` 섹션이 없으면 (investigate 미실행) → **이 Phase를 건너뛴다**
...
- **d:api 자동 수정 금지**: Phase 3-3의 d:api 제안은 반드시 사용자 확인 후에만 domain-map.ttl에 반영한다
- **triage-signals.md 자동 수정 금지**: Phase 4의 분류 조정 제안은 반드시 사용자 확인 후에만 반영한다
