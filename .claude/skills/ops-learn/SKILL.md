...
#### ② COOKBOOK.md (S3, S4, S5, S7 해당 시)

cookbook-rules.md의 구조와 규칙을 따라, triage 시그널에 따라 갱신:
- **S3 (해결 방안)**: 진단 체크리스트 추가/보강, 과거 사례 추가
- **S4 (진단 경로)**: 조사 플로우 추가 — 1단계 진단이라도 그 방법이 정답 경로면 기록
- **S5 (기존 패턴 동일)**: 해당 플로우의 히트 +1, 출처 이슈 추가만 (새 항목 추가 없음)
- **S7 (SQL 패턴)**: SQL 템플릿 추가
- API 스펙/제약 추가
