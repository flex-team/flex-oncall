---
name: ops-learn
description: Use when any knowledge source (Notion docs, Slack threads, completed Linear tickets, or local operation-notes) needs to be internalized into the brain system — updates COOKBOOK, CLAUDE.md submodule map, domain-map.ttl, and reference memory in one pass. Triggers include '학습해줘', '이 문서 학습', '데이터화해줘', 'learn this', sharing a Notion/Slack link for knowledge capture, '쿡북 업데이트', '아티팩트 갱신', or after note-issue/investigate-issue/close-note completes. Supports multiple sources in a single invocation.
...
지식 소스에서 운영에 유용한 정보를 추출하여 brain 산출물(COOKBOOK, CLAUDE.md, domain-map.ttl, memory)을 갱신한다.
...
| S8 | 도메인 이해/개념 | `cookbook/{domain}.md` "도메인 컨텍스트" 갱신 | 조사 과정에서 "왜 이런 스펙인지" 배경이 밝혀짐 |
...
추출된 항목들을 시그널 매트릭스에 대조
  ...
  S1-S4, S6-S8 중 하나 이상 → 해당 목적지에만 정확히 갱신
...
triage_result:
  ...
  targets:                        # Phase 4에서 갱신할 산출물과 내용
    ...
    context: [{section: "비즈니스 규칙|자주 혼동되는 것들|구현 특이사항", content: "..."}]
    domain_map: [{field: "d:kw|d:syn|g:*|d:x", value: "..."}]
...
`brain/cookbook/{domain-id}.md`의 "도메인 컨텍스트" 섹션을 갱신한다.

- 섹션이 없으면 → 아래 템플릿으로 기존 내용 위에 생성 후 `---` 구분선으로 분리
  ```markdown
  ## 도메인 컨텍스트
  ### 이 도메인이 하는 일
  ### 핵심 개념
  ...
  ---
  ```
- 있으면 → triage의 `context` 항목에 따라 해당 하위 섹션에 추가/갱신
- 주 갱신 대상: "구현 특이사항", "자주 혼동되는 것들", "비즈니스 규칙"
- 코드에서 유도할 수 없는 "왜"(법적 근거, 비즈니스 배경, 설계 의도)에 집중
- 스펙 복사가 아닌 배경만 기록 — 상세 규칙은 `brain/cookbook/README.md` 참조
...
### 산출물 갱신 순서
- ①~③은 순서 유연하나, ④ domain-map.ttl은 반드시 마지막
