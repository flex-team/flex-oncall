...
- 도메인 블록의 `d:syn` 에 새 사용자 표현 문장 추가 (GLOSSARY `g:q`에서 추출한 2어절 이상 문장)
...
`brain/routing-misses.md` 에 미처리 항목이 있으면:

1. 파일을 읽어 "최종 도메인" 칸이 채워진 행을 찾는다
2. 각 행의 "입력 텍스트"에서 키워드를 추출
3. 해당 도메인의 `d:kw` 또는 `d:syn` 에 누락된 키워드가 있으면 추가
4. 처리 완료된 행은 테이블에서 **삭제** (처리 이력은 domain-map.ttl 갱신으로 남음)
5. "최종 도메인"이 비어있는 행은 그대로 유지 (수동 확인 필요)
