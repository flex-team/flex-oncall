...
operation-note와 Linear 이슈에서 문제의 전체 그림을 파악한다.
...
4. 사실 기반 문제 검증 (API 관련 이슈일 때):
   - 관련 API/코드 식별
   - 코드 확인 → 현재 동작이 의도된 스펙인지 초기 판단
   - access log 조회로 실제 동작 확인 (필요 시)
   - 문의 내용 vs 실제 동작 대조 → 문제 정의 보정

   수행 조건:
   - API 요청/응답이 문제의 핵심인 경우
   - 문의 내용만으로 실제 동작을 특정하기 어려운 경우

   건너뛰는 조건:
   - 문제가 이미 명확한 경우 (스크린샷 등)
   - 배치/스케줄러 등 백그라운드 처리 이슈
