...
**코드를 깊이 파기 전에, 유사 이슈와 최근 변경 이력을 먼저 확인한다.** 이미 해결되었거나 관련 수정이 진행 중인 경우가 많다.

1. **Linear 유사 이슈 검색** (Linear MCP `list_issues` / `research` 활용):
   - CI 팀 이슈 중 동일 서비스 영역(라벨)의 최근 이슈 검색
   - 증상 키워드(에러 메시지, 화면명, 기능명)로 검색
   - 유사 이슈를 찾으면:
     - **Done**: 과거 해결 방법을 참고하여 동일 원인인지 확인 → 동일하면 "이미 해결됨" 경로
     - **In Progress / Pending**: 다른 담당자가 이미 조사 중 → 중복 작업 방지, 해당 스레드에 합류
     - **Todo**: 동일 이슈가 여러 건 접수된 상황 → 영향 범위가 넓을 수 있음, 긴급도 재평가
2. **git log 검색**: 해당 도메인 디렉토리의 최근 1-2주 커밋/PR 이력 확인
   - `git log --oneline --since="2 weeks ago" -- <도메인 디렉토리>`
3. **관련 PR 검색**: GitHub에서 해당 도메인 키워드로 최근 merged/open PR 확인
4. **슬랙 스레드 이력**: 동일 도메인의 최근 이슈 스레드에서 유사한 문의/핫픽스 이력 확인
...
조사에 사용할 수 있는 도구와 적합한 상황을 정리한다. 신규 온콜 담당자는 이 테이블을 참고하여 "무엇으로 확인해야 하는지"를 결정한다.
