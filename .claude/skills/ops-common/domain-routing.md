...
`ops-find-domain`, `ops-note-issue`, `ops-investigate-issue` 등 도메인 특정이 필요한 모든 ops 스킬이 이 가이드를 참조한다.
...
| 블록 타입 | 접두사 | 매칭 대상 필드 |
|----------|--------|--------------|
| Domain | `:` | `d:n`(이름), `d:kw`(키워드), `d:mod`(모듈명) |
| Note | `n:` | `d:s`(요약) |
...
| `g:q` (용어집 질문) | 2 | 사용자 표현과 직접 매칭 |
| `g:a` (용어집 답변) | 2 | 시스템 용어 매칭 (예: SES, preference=ON_TIME) |
| `d:s` (노트 요약) | 2 | 과거 이슈 패턴 매칭 |
| `d:n` (도메인 이름) | 1 | 넓은 매칭 |
