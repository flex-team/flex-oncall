...
$ARGUMENTS — `/oncall-investigate`의 Output, `/oncall-triage`의 Output, 또는 직접 입력. 아래 정보가 필요하다:
...
- 대화 컨텍스트에 investigate 결과가 있으면 그대로 사용한다.
- 직접 호출 시 위 필드를 제공하면 된다. 원인 분류가 없으면 어떤 포맷이 필요한지 물어본다.
...
1. 아래 포맷에 맞춰 메시지 내용을 먼저 생성한다.
2. triage 결과에 **슬랙 스레드 정보**(channel_id, thread_ts)가 있으면 해당 스레드에 답장 초안으로 생성한다. 채널/스레드 확인을 사용자에게 다시 묻지 않는다.
3. 슬랙 스레드 정보가 없으면, 사용자에게 전송할 채널을 확인한다 (예: "어느 채널에 초안을 생성할까요?"). 이전 대화에서 채널이 이미 특정된 경우 재확인 없이 진행한다.
4. 채널명만 알고 있으면 `slack_search_channels`를 호출하여 channel_id를 조회한다.
5. `slack_send_message_draft`로 초안을 생성한다. thread_ts가 있으면 함께 전달하여 스레드 답장으로 생성한다.
6. 생성된 초안 링크를 사용자에게 공유한다.
