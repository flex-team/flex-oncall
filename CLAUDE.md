...
**도메인 지식(Knowledge Base 등)의 위치 원칙**:
- 도메인 지식은 **해당 도메인 repo에** 둔다 — 코드가 바뀌면 지식도 같이 바뀌어야 하므로
- 도메인 repo의 CLAUDE.md에 KB 경로가 안내되어 있으면, investigate가 서브모듈 탐색 시 자연스럽게 발견한다
- support-oncall에 특정 도메인의 KB 링크나 전용 인프라를 두지 않는다 — 위치가 바뀔 때 감지 못 하고 부패한다
