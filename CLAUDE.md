...
>
> **DB 조회를 실행할 수 없는 경우**: `ops-db-query-builder` 의 도메인 라우팅 → Entity 탐색 → SQL 구성은 동일하게 수행하고, 실행만 `ops-common/delegation-guide.md` 의 위임 포맷으로 대체한다.
> - Entity 출처 기반 SQL 구성 원칙은 동일하게 적용
> - 인덱스 확인은 JPA 어노테이션 + 마이그레이션 파일 기반으로 추론하고 `[JPA 추론]` 태그 부착
