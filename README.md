...
서브모듈은 매일 KST 08:00에 GitHub Action(`sync-submodules`)이 각 서브모듈의 추적 브랜치 최신 커밋으로 갱신하여 main에 자동 커밋한다. 따라서 로컬에서는 `git pull`만 하면 항상 최신 코드베이스를 탐색할 수 있다.

이슈가 인입되면 이 repo를 시작점으로 문제를 파악한다:

```mermaid
flowchart TD
    A[이슈 인입<br/>Linear / Slack] --> B[도메인 파악<br/>GLOSSARY.md · 키워드 매핑]
    B --> C[관련 서브모듈 특정<br/>CLAUDE.md 서브모듈 맵]
    C --> D[과거 사례 확인<br/>COOKBOOK.md · INDEX.md]
    D --> E{직접 해결 가능?}
    E -->|Yes| F[코드 조사<br/>서브모듈 내 탐색]
    E -->|No| G[담당 팀 라우팅<br/>도메인별 슬랙 멘션]
    F --> H[원인 분석 · 해결]
    H --> I[운영 노트 기록<br/>operation-notes/]
```
