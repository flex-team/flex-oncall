...
> {문제 성격 한 줄} · {범위 한 줄} · {긴급도}

- **성격**: {자유 서술} — 근거: {근거}
- **범위**: {누가 얼마나} — 근거: {쿼리/분석}
  - 진행형: {여부}
- **긴급도**: {판단} — 근거: {근거}
- **조사 방향**: {우선 확인 대상} / COOKBOOK: {F번호 또는 "해당 없음"}
...
- 수정 시 사이드이펙트 포인트 (버그 판정 시만, ops-impact-analyze가 작성)

## 해결안

- 즉시 대응 vs 근본 해결 옵션 (버그 판정 시만, ops-impact-analyze가 작성)

## 영향 범위

- 이 이슈를 겪을 수 있는 사용자/회사 범위
- 금전적·법적 영향이 있는가 (있다면 도메인별 추가 섹션 참조)

<!-- 도메인별 추가 섹션: domain-map.ttl의 d:tpl 속성에 따라
     brain/note-templates/{tpl}.md의 추가 섹션이 여기에 삽입됨.
     d:tpl이 없으면 이 위치에 추가 섹션 없음. -->
