...
| `d:st` | lifecycle status (농축 상태) | enum string | note |
| `d:ca` | consolidated-at (농축 완료일) | date string (`"YYYY-MM-DD"`) | note |
| `d:q` | user query (사용자 표현) | string (다중) | glossary |
| `d:a` | answer (시스템 용어) | string | glossary |
| `d:api` | 도메인의 주요 API 엔드포인트 prefix. access log 검색 시 사용. 상세 엔드포인트는 코드에서 확인. | string (다중, 쉼표 구분) | domain |
| `d:tpl` | 노트 템플릿 유형. `ops-note-issue`가 노트 생성 시 `brain/note-templates/{tpl}.md`를 참조. 값이 없으면 `_default.md` 사용. | string | domain |
...
:new-domain
    ...
    d:mod "/모듈경로" ;
    d:kw "키워드1", "키워드2" ;
    d:syn "사용자 표현 문장1", "사용자 표현 문장2" .
```

> `d:syn`은 문장형 사용자 표현을 도메인에 직결한 것이다. `d:kw`가 단어형 키워드를 담당하고, `d:syn`은 문장형 사용자 표현을 담당한다. 농축(ops-close-note, ops-compact) 시 노트에서 추출되어 추가된다.
...
### lifecycle status 값 (열거형)
| 값 | 의미 |
|----|------|
| `"C"` | consolidated (농축 완료 — 유효 신호가 domain-map.ttl의 keyword/synonym과 COOKBOOK에 흡수됨) |

- 노트가 close 되어 archive로 이동한 후, 농축이 완료되면 `d:st "C"` 와 `d:ca "YYYY-MM-DD"` 를 함께 설정한다.
- `d:st` 가 없는 archive 노트는 아직 농축되지 않은 것이다.
- `d:ca` 날짜 + 90일 경과 시 퇴출(n:* 트리플 삭제) 대상이 된다.
- `d:ca` 가 없는 `d:st "C"` 노트는 농축 시점 불명으로 퇴출 대상에서 제외된다.
...
- **해당 도메인에 고유한 용어만** 키워드로 등록한다. 여러 도메인에 공통으로 쓰이는 일반 용어는 등록하지 않는다.
  - ❌ `soft-delete`, `벌크`, `취소`, `삭제`, `감사로그`, `validation`, `sync` — 범용 패턴이라 라우팅 노이즈 유발
  - ✅ `근무유형`, `연차촉진`, `holidayProps`, `WORKRULE_400_005` — 특정 도메인에서만 의미를 갖는 용어
- 판단 기준: **이 키워드를 들었을 때 하나의 도메인이 바로 특정되는가?** 2개 이상 도메인에 해당되면 키워드가 아니라 쿡북/용어집에서 맥락으로 구분해야 한다.
