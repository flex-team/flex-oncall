...
소수점은 ADJUST 버킷이 생성된 시점부터 **잠재적으로 존재**하지만, 우연히 두 버킷의 분수가 합산 시 깔끔하게 상쇄되면 눈에 보이지 않는다.

```
사용 전 — 13.5일로 표시 (분수가 서로 상쇄)
  REGULAR: 6080분 ÷ 480 = 38/3 = 12.666...
  ADJUST:   300분 ÷ 360 =  5/6 =  0.833...
  합산: 38/3 + 5/6 = 81/6 = 13.5 (정확히 맞아 떨어짐)

사용 후 — 13.002일로 표시 (상쇄 관계가 깨짐)
  REGULAR: 5841분 ÷ 480 = 12.16875  (분자가 바뀌어 더 이상 깔끔하지 않음)
  ADJUST:   300분 ÷ 360 =  0.83333...
  합산: 13.00208... → setScale(3, HALF_UP) = 13.002
```

소정근로시간 혼재가 있는 유저는 연차 사용·조정 이벤트 이후 이런 소수점이 갑자기 나타날 수 있다.
