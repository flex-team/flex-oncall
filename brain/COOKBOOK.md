...
#### 진단 체크리스트 (구성원 일괄 업로드 후 데이터 누락)
문의: "구성원 일괄 업로드했는데 조직/직군/주민번호/계좌가 안 들어갔어요" / "업로드 완료됐다고 나왔는데 정보가 없어요"
1. **엑셀 직군명 검증** → DB에 등록된 직군 이름과 정확히 일치하는지 확인 (공백, 특수문자 포함) [CI-4355]
   - 직군명 불일치 시 해당 row의 조직·직군 필드 전체가 미저장됨
2. **계좌번호 형식 검증** → 계좌번호에 하이픈(`-`) 포함 여부 확인 [CI-4355]
   - 하이픈 포함 시 해당 row의 주민번호·계좌 필드 전체가 미저장됨
3. **완료 알림의 맹점** → `sendNotification()`이 USER 타입만 카운트하여 '완료'로 표시됨 [CI-4355]
   - personnelAppointment/basic 저장 실패해도 알림은 '완료'로 발송
4. **대응**: 고객에게 직군명 정확 입력 + 계좌번호 하이픈 제거 후 재업로드 안내
...
1. **cross-year 트리 구조 확인** → 사용자가 이전 연도 root 목표 하위에 올해 자식 목표를 배치했는지 확인 [CI-4126] [CI-4358]
   - `root-objectives` API는 올해 목표의 root를 트리 탐색으로 찾으므로, root가 이전 연도면 `hit=false`로 포함
   - FE는 `hit=false` 목표를 **회색으로 구분 표시** — 이는 의도된 스펙
   - ⚠️ **뎁스 주의**: 올해 자식 목표가 root 바로 아래가 아니라 깊은 뎁스에 있을 수 있다. DB 확인 시 `root_objective_id`(트리 소속)뿐 아니라 `parent_id`(직접 부모)도 확인하여 정확한 위치를 안내해야 한다 [CI-4358]
2. 회색 목표의 의미 → "직접 필터에 해당하지는 않지만, 하위 **어딘가에** 해당하는 목표가 있어서 관계성을 보여주기 위해 표시"
   - 💡 **UI 팁**: 4depth 이상은 펼치기 안내가 나오지만, **Cmd + 클릭**으로 하위 전체를 한번에 펼칠 수 있다. 깊은 뎁스의 cross-year 목표를 고객이 직접 확인할 때 안내하면 유용
