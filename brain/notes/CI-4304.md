...
| 방향 | 내용 | 수정 대상 |
|---|---|---|
| 클린업 범위 복원 | 구 규칙 ID도 클린업 대상에 포함 | `OsPeriodicWorkScheduleCleansingSyncService` |
| 조회 방어 필터 | `startDateOfPeriod.dayOfMonth` 검증 추가 | `OsPeriodicWorkScheduleSearchChildCondition` |

둘 다 적용하는 것이 안전하다. 클린업만으로는 순서 문제(클린업 → re-sync)가 남고, 조회 필터만으로는 기존 고스트가 계속 쌓인다.
