...
- Linear 코멘트에 추가 맥락 없음 (bot 동기화 코멘트만 존재)
- Slack 스레드에서 추가 맥락 확인 가능[^2]

## 원인 분석

**스펙 (의도된 설계)** — 서비스 경계 분리에 의한 비대칭 검증 구조.
...
**근거**: 시스템은 **"Prevention forward"**(새로운 액션만 사전 차단하고, 기존 데이터를 소급 변경하지 않는 검증 패턴) 을 사용한다. 휴직이 등록되면 그 이후부터 휴가·출퇴근·근무일정 등록이 차단되지만, 이미 존재하는 데이터를 소급 취소하지는 않는다. 이는 서비스 간 결합도를 낮추고 데이터 정합성 사고를 방지하기 위한 설계로 판단된다.
...
1. **의도된 스펙임을 안내**: 휴직 등록과 휴가 등록은 별도 서비스에서 관리하며, 검증 방향이 다름
2. **운영 가이드**: 휴직 등록 전 해당 기간의 기존 휴가를 먼저 취소/조정한 뒤 휴직을 등록하도록 안내
3. **검증 방향 요약**:
   - 휴가 등록 시 → 휴직 기간 체크 ✅ (차단됨)
   - 휴직 등록 시 → 기존 휴가 체크 ❌ (허용됨, 스펙)
