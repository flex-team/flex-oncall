...
| [CI-4158](./CI-4158.md) | 삭제한 평가 리스트 재노출 — 스펙 (DRAFT 정상 노출, FE 핫픽스) | 평가, DRAFT, 삭제, 재노출, title null, soft-delete, draft_evaluation, FE 핫픽스, 스펙 |
| [CI-4164](./CI-4164.md) | 평가 "지금 시작" 클릭 시 예약 상태로 저장 — FE/BE 시간차 버그 | 평가, 시작, 예약, RESERVED, IN_PROGRESS, period.start, 클라이언트 시간, EvaluationStepProgress, 크론 |
...
### 신고/세금 (Tax / Social Insurance)

| 문서 | 요약 | 키워드 |
|------|------|--------|
| [CI-4159](./CI-4159.md) | 연속 휴직 월 중간 경계 분리 시 근무월수 오집계 수정 | 사회보험, 연말정산, 건강보험, 근무월수, 휴직, 연속휴직, HealthInsuranceMonthsCalculator, calculateWorkingMonths |

### 인증 (Authentication)

| 문서 | 요약 | 키워드 |
|------|------|--------|
| [CI-4166](./CI-4166.md) | 계열사 전환 시 세션 시간 차이로 로그인 풀림 — 스펙 | 계열사, 전환, 로그인, 세션, 자동 로그아웃, exchange API, 401, AUTH_401_000, 겸직 |

### 채용 (Recruitment)

| 문서 | 요약 | 키워드 |
|------|------|--------|
| [CI-4170](./CI-4170.md) | 채용사이트 subdomain 변경 신청 검토중 방치 — operation API로 수동 승인 | 채용, subdomain, 도메인 변경, 채용사이트, operation API, 수동 승인, site_subdomain_change, alarm-recruiting-operation |
